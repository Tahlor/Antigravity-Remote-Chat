...
export async function findAntigravityProcess() {
    ...
    try {
        const candidates = await findProcessCandidates(processName);
...
async function findProcessCandidates(name) {
...
function parsePortOutput(stdout, pid) {
    const ports = [];
    if (process.platform === 'win32') {
...
function testPortWithProtocol(port, csrfToken, protocol) {
